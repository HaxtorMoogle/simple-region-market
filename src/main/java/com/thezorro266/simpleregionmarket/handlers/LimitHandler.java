...
public class LimitHandler {
	...
	private YamlConfiguration limitConfig;
	...
	public int countPlayerTokenRegions(TemplateMain token, Player player) {
		...
		for(String world : token.entries.keySet()) {
			for(String region : token.entries.get(world).keySet()) {
				ProtectedRegion protectedRegion = SimpleRegionMarket.getWorldGuard().getRegionManager(Bukkit.getWorld(world)).getRegionExact(region);

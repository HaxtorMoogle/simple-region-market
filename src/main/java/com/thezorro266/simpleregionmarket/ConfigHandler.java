...
public class ConfigHandler {
  ...
  public void load() {
    final YamlConfiguration confighandle = YamlConfiguration.loadConfiguration(agents);
    ...
    for (final String world : confighandle.getKeys(false)) {
      ...
      for (final String region : path.getKeys(false)) {
        ...
        for (final String signnr : path.getKeys(false)) {
          ...
          final ProtectedRegion protectedregion_region = SimpleRegionMarket.getWorldGuard().getRegionManager(world_world).getRegion(region);

          SignAgent newagent;
          if (expiredate != null) {
            newagent = plugin.getAgentManager().addAgent(path.getInt("Mode", 0),
                new Location(world_world, path.getDouble("X", 0), path.getDouble("Y", 0), path.getDouble("Z", 0)), protectedregion_region,
                path.getDouble("Price"), account, path.getLong("RentTime", -1), path.getString("RentBy"), expiredate);
          } else {
            newagent = plugin.getAgentManager().addAgent(path.getInt("Mode", 0),
                new Location(world_world, path.getDouble("X", 0), path.getDouble("Y", 0), path.getDouble("Z", 0)), protectedregion_region,
                path.getDouble("Price"), account, path.getLong("RentTime", -1));
          }
          ...
          if (newagent == null) {
            if (world_world == null) {
              ...
              langHandler
                  .outputConsole(Level.WARNING, "Agent '" + path.toString() + "' from 'agents.yml' was not loaded properly. Adding to 'agents_fail.yml'");

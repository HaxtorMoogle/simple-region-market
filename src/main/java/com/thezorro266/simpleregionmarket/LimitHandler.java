...
public class LimitHandler {
	...
	public int countPlayerOwnRegion(Player p) {
		if (p != null) {
			...
			return tmp.getGlobalRegionManager().get(p.getWorld())
					.getRegionCountOfPlayer(tmp.wrapPlayer(p));
		}
		return 0;
	}
	...
	public boolean limitCanBuy(Player p) {
		if (p != null) {
			...
			if (limitregionplayers.containsKey(playername))
				return countPlayerOwnRegion(p) < limitregionplayers
						.get(playername);
			else if (p.getWorld() != null
					&& limitregionworlds.containsKey(worldname)
					&& limitregionworlds.get(worldname) != -1)
				return countPlayerOwnRegion(p) < limitregionworlds
						.get(worldname);
	...
	public boolean limitCanRent(Player p) {
		if (p != null) {
			...
			if (limitroomplayers.containsKey(playername))
				return countPlayerRentRoom(p) < limitroomplayers
						.get(playername);
			else if (p.getWorld() != null
	...
	public void loadLimits() {
		limitConfig = YamlConfiguration.loadConfiguration(new File(
				SimpleRegionMarket.plugin_dir + "limits.yml"));
		...
		for (final String main : limitConfig.getKeys(false)) {
			...
			for (final String limiter : path.getKeys(false)) {
				...
				path = limitConfig.getConfigurationSection(main)
						.getConfigurationSection(limiter);
				for (final String key : path.getKeys(false)) {
					if (main.equalsIgnoreCase("regions")) {
						if (limiter.equalsIgnoreCase("worlds")) {
							limitregionworlds.put(key.toLowerCase(),
									path.getInt(key));
						} else if (limiter.equalsIgnoreCase("players")) {
							limitregionplayers.put(key.toLowerCase(),
									path.getInt(key));
						}
					} else if (main.equalsIgnoreCase("rooms")) {
						if (limiter.equalsIgnoreCase("worlds")) {
							limitroomworlds.put(key.toLowerCase(),
									path.getInt(key));
						} else if (limiter.equalsIgnoreCase("players")) {
							limitroomplayers.put(key.toLowerCase(),
									path.getInt(key));
						}
					}
				}
	...
	public void saveLimits() {
		...
		for (final String key : limitregionworlds.keySet()) {
			limitConfig
					.set("regions.worlds." + key, limitregionworlds.get(key));
		}

		limitConfig.createSection("regions.players");
		for (final String key : limitregionplayers.keySet()) {
			limitConfig.set("regions.players." + key,
					limitregionplayers.get(key));
		}
